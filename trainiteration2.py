...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep  7 13:09:56 2017
...
class trainmain():
    def __init__(self,model,optimizer,trainData,iscuda=True,is1D=True,nplot=8,logfile='',iscontinue=False):
        ...
        if not iscontinue:    
            self.savedpara = {"mu":np.zeros((len(self.trainData),self.ndf)),\
                            "logvar":np.zeros((len(self.trainData),self.ndf)),\
                            "loss":np.zeros((len(self.trainData))),\
                            "BCE":np.zeros((len(self.trainData))),\
                            "KLD":np.zeros((len(self.trainData)))}
...
# For dose
    ...
    def train(self,epoch,isTrain=False,noise_factor=0.5,issave=False,isplot=True,pltfile=[],islog=True):
        ...
        train_BCE = 0
        train_KLD = 0
        ntrain = len(self.trainData)
        
        for batch_idx, (data, _) in enumerate(self.trainData):
            nbatch = data.size()[0]
            data = Variable(data)
...
#            data_noise.data.clamp_(0., 1.)
            ...
            if isplot:
                if self.is1D:
                    ...
                    self.plotrecon2D(data,pltfile)
                    
                    pltfilerecon = re.sub('.eps','_recon.eps',pltfile)
                    self.plotrecon2D(recon_batch,pltfilerecon)
